...
void draw_font(Uint16 col_address, Uint16 low_address, Uint16 font,Uint32 color, Uint16 cs_flag)
{
...
#ifdef LCD_262K_COLOR_MODE
	Blue = (color&0x003F0000)>>16; 
#else
    ...
    for(i=0; i<FONT_BITMAP_SIZE; i++)
    {
        ...
        for(j=0; j<8; j++)
        {
            ...
            {
#ifdef 	LCD_262K_COLOR_MODE
				dataWrite(Blue, cs_flag);
				dataWrite(Green, cs_flag);
				dataWrite(Red, cs_flag);
#else
