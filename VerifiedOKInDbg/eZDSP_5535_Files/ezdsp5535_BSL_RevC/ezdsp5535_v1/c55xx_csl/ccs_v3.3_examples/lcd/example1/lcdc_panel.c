          ...
          Uint16 cs_flag)
{
	...
	cmdWrite(0xAE, cs_flag);    // sleep mode on
		...
		dataWrite(0xB1, cs_flag);   // 0xb1: 262K color, vertical 0x71: 65K color, vertical
#else
		dataWrite(0x70, cs_flag);   // 0xb1: 262K color, vertical 0x71: 65K color, vertical
#endif
...
** 	Description : 	- The function draw a line between the specified points **
...
#else
    Green = (color)>>8; // b4 b3 b2 b1 b0 g5 g4 g3
    Red = (color&0x0000FF);    // g2 g1 g0 r4 r3 r2 r1 r0
#endif    
    Width = colEnd - colStart+1;
    Height = rowEnd - rowStart+1;
    
    m  = (float) (Height-1) / (float) (Width-1);
	b = rowStart - m * colStart;
	dx =  (colEnd > colStart) ? 1 : -1;

    if((Width < 0) || (Height < 0))
        return LCDC_E_INVAPARAMS;
    while (colStart != colEnd) {
		colStart += dx;
		rowStart = m*colStart + b;
		cmdWrite(COLUMN_RAM_ADDRESS, cs_flag);
		dataWrite(colStart, cs_flag);
		dataWrite(colStart, cs_flag);        
...
#if defined(LCD_262K_COLOR_MODE)
           dataWrite(Blue, cs_flag);      // B
           dataWrite(Green, cs_flag);     // G
           dataWrite(Red, cs_flag);       // R
#else
		   dataWrite(Green, cs_flag);	// b4 b3 b2 b1 b0 g5 g4 g3
		   dataWrite(Red, cs_flag);		// g2 g1 g0 r4 r3 r2 r1 r0
#endif
	}
	return 0;
}
               ...
               Uint16     cs_flag )
{
    ...
    if ( (colEnd < colStart ) || (rowEnd < rowStart) )
	 ...
	 dataWrite(tempLSB, cs_flag);
     dataWrite(tempMSB, cs_flag);
	 tempLSB = (Uint8)(fillColor & 0xFF);
	 tempMSB = (Uint8)((fillColor >> 8) & 0xFF);
	 dataWrite(tempLSB, cs_flag);
     dataWrite(tempMSB, cs_flag);
 ...
 *  \return Int16  status of call, whether a success(0) or not(other than 0)
 */
Int16 draw_ti_logo(Uint16 cs_flag)
{
	Uint16  i, j;
	...
	// Red
   	cmdWrite(COLUMN_RAM_ADDRESS, cs_flag);		    // Set Column Address
    dataWrite(0x0, cs_flag);		     	// Default => 0x00 (Start Address)
    dataWrite(0x3f, cs_flag);			// Default => 0x7F (End Address)		
    ...
    cmdWrite(ROW_RAM_ADDRESS, cs_flag);			// Set Row Address
	dataWrite(30, cs_flag);				// Default => 0x00 (Start Address)
    dataWrite(0x7f, cs_flag);			// Default => 0x7F (End Address)	  	
