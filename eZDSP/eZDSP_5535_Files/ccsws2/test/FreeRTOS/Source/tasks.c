...
typedef struct tskTaskControlBlock
{
	...
	portSTACK_TYPE			*pxSysStack;			/*< Points to the start of the stack. */
	signed char				pcTaskName[ configMAX_TASK_NAME_LEN ];/*< Descriptive name given to the task when created.  Facilitates debugging only. */

	#if ( portSTACK_GROWTH > 0 )
	portSTACK_TYPE *pxEndOfStack;			/*< Used for stack overflow checking on architectures where the stack grows up from low memory. */
	portSTACK_TYPE *pxEndOfSysStack;			/*< Used for stack overflow checking on architectures where the stack grows up from low memory. */
	#endif
...
//	struct tagSTACKSTRUCT *stackStruct;
	...
	if( pxNewTCB != NULL )
	{
		...
		#if( portSTACK_GROWTH < 0 )
		{
			...
			pxTopOfSysStack = pxNewTCB->pxSysStack + ( usStackDepth - 1 );
			pxTopOfSysStack = ( portSTACK_TYPE * ) ( ( ( unsigned long ) pxTopOfSysStack ) & ( ( unsigned long ) ~portBYTE_ALIGNMENT_MASK  ) );
		}
		#else
		{
			pxTopOfStack = pxNewTCB->pxStack;
			pxTopOfSysStack = pxNewTCB->pxSysStack;

			/* If we want to use stack checking on architectures that use
			a positive stack growth direction then we also need to store the
			other extreme of the stack space. */
			pxNewTCB->pxEndOfStack = pxNewTCB->pxStack + ( usStackDepth - 1 );
			pxNewTCB->pxEndOfSysStack = pxNewTCB->pxSysStack + ( usStackDepth - 1 );
		}
		#endif
		...
		#else
		{
//			stackStruct = pxPortInitialiseStack( pxTopOfStack, pxTopOfSysStack, pxTaskCode, pvParameters );
			pxPortInitialiseStack( pxTopOfStack, pxTopOfSysStack, pxTaskCode, pvParameters );
			pxNewTCB->pxTopOfStack = stackStruct->pxTopOfStack;
			pxNewTCB->pxTopOfSysStack = stackStruct->pxTopOfSysStack;


// #if ( portUSING_SYSSTACK == 1 )
			
//			 = pxNewTCB->pxTopOfStack;	// just init to the same - initially
// #endif
...
// should only reach here if kernel could not be started
	while (1) {
...
tskTCB *pxNewTCB;
	...
	if( pxNewTCB != NULL )
	{
		...
		pxNewTCB->pxSysStack = ( portSTACK_TYPE * ) pvPortMallocAligned( ( ( ( size_t )usStackDepth ) * sizeof( portSTACK_TYPE ) ), puxStackBuffer );

		if( pxNewTCB->pxStack == NULL || pxNewTCB->pxSysStack == NULL )
		{
		...
		else
		{
			...
			memset( pxNewTCB->pxSysStack, tskSTACK_FILL_BYTE, usStackDepth * sizeof( portSTACK_TYPE ) );
		}
	}
...
//				usStackRemaining = usTaskCheckFreeStackSpace( ( unsigned char * ) pxNextTCB->pxSysStack );
			}
			#endif			
...
/*-----------------------------------------------------------*/
	...
	static void prvDeleteTCB( tskTCB *pxTCB )
	{
		...
		vPortFreeAligned( pxTCB->pxSysStack );
		vPortFree( pxTCB );
	}
